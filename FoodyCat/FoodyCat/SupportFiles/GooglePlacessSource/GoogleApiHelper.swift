...
class GApiResponse {
    ...
    class Autocomplete {
        ...
        class func initWithData(_ data:[String:Any]) -> Autocomplete {
            ...
            if let pId = data["place_id"] as? String,let address = data["description"] as? String,let distance = data["distance_meters"] as? Double {
                object.formattedAddress = address
                object.placeId = pId
...
class GoogleApi : NSObject {
    ...
    func getConvertedInputFor(_ usedFor:UsedFor,input:GInput) -> String {
        switch usedFor {
        case .autocomplete:
            let originString = "&origin=\(input.originCoordinate!.latitude!),\(input.originCoordinate!.longitude!)&radius=50000"
            
            return input.keyword! + originString
        case .placeInformation:
        ...
        case .path:
            ...
            if let oCoordinate = input.originCoordinate, let dCoordinate = input.destinationCoordinate{
                ...
                if let wayPoints = input.wayPoints {
                    convertedInput = convertedInput + "&" + "waypoints="
                    for wayPoint in wayPoints {
                        if let lat = wayPoint.latitude, let lng = wayPoint.longitude {
                            let lat = String(format:"%.14f", lat)
                            let long = String(format:"%.14f", lng)
                            convertedInput.append(lat + "," + long)
                            convertedInput.append("|")
                        }
                    }
                    convertedInput.remove(at: convertedInput.index(before: convertedInput.endIndex))
                }
            }
        ...
        case .nearBy:
            ...
            var finalParameters = ""
            if let lat = input.originCoordinate?.latitude, let lng = input.originCoordinate?.longitude {
                let lat = String(format:"%.14f", lat)
                let long = String(format:"%.14f", lng)
                    ...
                    if error != nil || isApiError {
                        ...
                        if let predictions = responseData?["predictions"] as? [Results], api == .autocomplete {
                            ...
                            if let oPolyline = route["overview_polyline"] as? Results,let points = oPolyline["points"] as? String {
                                ...
                                var totalDistance = 0
                                var totalDuration = 0
                                if let legs = route["legs"] as? [Results]{
                                    
                                    for leg in legs {
                                        let dataDistance = leg["distance"] as? Results
                                        let distanceValue = dataDistance!["value"] as! Int
                                        
                                        //                                        //print(distanceValue)
                                        
                                        
                                        let dataDuration = leg["duration"] as? Results
                                        let durationValue = dataDuration!["value"] as! Int
                                        
                                        //                                        //print(durationValue)
                                        //data["distance"] = dataDistance
                                        totalDistance = totalDistance + distanceValue
                                ...
                                //                                //print(totalDuration)
                                //                                //print(totalDistance)
                                //                                //print(totalDuration / 1000)
                                //                                //print(totalDistance / 60)
                                //
                                response.totalDistance = Double(totalDistance) // / 1000.0
                                response.totalDuration = totalDuration / 60
